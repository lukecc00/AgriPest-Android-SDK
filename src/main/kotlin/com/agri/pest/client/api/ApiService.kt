package com.agri.pest.client.api

import com.agri.pest.client.model.request.*
import com.agri.pest.client.model.response.*

import org.openapitools.client.infrastructure.CollectionFormats.*
import retrofit2.http.*
import retrofit2.http.HTTP
import okhttp3.RequestBody
import io.reactivex.rxjava3.core.Single
import io.reactivex.rxjava3.core.Completable
import com.google.gson.annotations.SerializedName


import okhttp3.MultipartBody
import retrofit2.http.Streaming
import okhttp3.ResponseBody

interface ApiService {
    /**
     * 发表评论
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @param commentRequest 
     * @return [Call]<[ResultCommentResponseDto]>
     */
    @POST("api/posts/{id}/comments")
    fun addComment(@Path("id") id: kotlin.Long, @Body commentRequest: CommentRequest): Single<ResultCommentResponseDto>

    /**
     * 添加种植标签 (浇水、施肥、用药、笔记)
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @param tagOperationRequest 
     * @return [Call]<[ResultVoid]>
     */
    @POST("api/my-crops/{id}/tags")
    fun addTag(@Path("id") id: kotlin.Long, @Body tagOperationRequest: TagOperationRequest): Single<ResultVoid>

    /**
     * 换绑手机号
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param changePhoneRequest 
     * @return [Call]<[ResultVoid]>
     */
    @PUT("api/users/me/phone")
    fun changePhone(@Body changePhoneRequest: ChangePhoneRequest): Single<ResultVoid>

    /**
     * 发起多模态诊断请求
     * 上传文本描述和可选的图片，结合地理位置与天气数据返回结构化诊断结果。
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param chatRequest 
     * @return [Call]<[ResultListDiagnosisItem]>
     */
    @POST("api/v1/agent/chat")
    fun chat(@Body chatRequest: ChatRequest): Single<ResultListDiagnosisItem>

    /**
     * 发起多模态诊断请求(带耗时剖析)
     * 返回诊断结果并附带服务端分段耗时，用于定位慢点。
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param chatRequest 
     * @return [Call]<[ResultChatProfileResponse]>
     */
    @POST("api/v1/agent/chat/profile")
    fun chatProfile(@Body chatRequest: ChatRequest): Single<ResultChatProfileResponse>

    /**
     * 发起多模态诊断请求(流式返回)
     * 以 SSE 方式流式返回模型输出；单模型为真流式，多模型为伪流式。
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param chatRequest 
     * @return [Call]<[kotlin.String]>
     */
    @POST("api/v1/agent/chat/stream")
    @Streaming
    fun chatStream(@Body chatRequest: ChatRequest): Single<okhttp3.ResponseBody>

    /**
     * 完成分片上传
     * 所有分片上传完成后，调用此接口合并分片并返回最终的文件URL。
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param multipartUploadCompleteRequest 
     * @return [Call]<[ResultString]>
     */
    @POST("api/files/multipart/complete")
    fun completeMultipartUpload(@Body multipartUploadCompleteRequest: MultipartUploadCompleteRequest): Single<ResultString>

    /**
     * 添加新记录的作物
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param myCropCreateRequest 
     * @return [Call]<[ResultMyCropResponseDto]>
     */
    @POST("api/my-crops")
    fun createCrop(@Body myCropCreateRequest: MyCropCreateRequest): Single<ResultMyCropResponseDto>

    /**
     * 发布新帖子
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param postCreateRequest 
     * @return [Call]<[ResultPostResponseDto]>
     */
    @POST("api/posts")
    fun createPost(@Body postCreateRequest: PostCreateRequest): Single<ResultPostResponseDto>

    /**
     * 管理员创建用户
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param adminUserUpdateDto 
     * @return [Call]<[ResultAdminUserResponseDto]>
     */
    @POST("api/admin/users")
    fun createUser(@Body adminUserUpdateDto: AdminUserUpdateDto): Single<ResultAdminUserResponseDto>

    /**
     * 管理员删除评论
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/admin/community/comments/{id}")
    fun deleteComment(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 删除作物
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/my-crops/{id}")
    fun deleteCrop(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 删除当前用户的识别历史记录
     * 根据历史记录 ID 删除当前登录用户自己的识别历史记录
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/v1/agent/history/{id}")
    fun deleteHistory(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 删除历史消息记录
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/admin/messages/{id}")
    fun deleteMessage(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 管理员删除帖子
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/admin/community/posts/{id}")
    fun deletePost(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 管理员删除用户
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/admin/users/{id}")
    fun deleteUser(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 收藏指定帖子
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @POST("api/posts/{id}/favorite")
    fun favoritePost(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 关注用户
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @POST("api/users/{id}/follow")
    fun followUser(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 获取当前用户的预警消息列表 (首页专用，仅未读)
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 5)
     * @return [Call]<[ResultPageResultMessageResponseDto]>
     */
    @GET("api/messages/alerts")
    fun getAlertMessages(@Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 5): Single<ResultPageResultMessageResponseDto>

    /**
     * 获取所有评论列表 (分页)
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultPageResultCommentResponseDto]>
     */
    @GET("api/admin/community/comments")
    fun getAllComments(@Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultPageResultCommentResponseDto>

    /**
     * 获取所有帖子列表 (分页)
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param tag  (optional)
     * @param q  (optional)
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultPageResultPostResponseDto]>
     */
    @GET("api/admin/community/posts")
    fun getAllPosts(@Query("tag") tag: kotlin.String? = null, @Query("q") q: kotlin.String? = null, @Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultPageResultPostResponseDto>

    /**
     * 获取帖子评论列表
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultPageResultCommentResponseDto]>
     */
    @GET("api/posts/{id}/comments")
    fun getComments(@Path("id") id: kotlin.Long, @Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultPageResultCommentResponseDto>

    /**
     * 获取作物详情 (包含种植日历记录)
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultMyCropResponseDto]>
     */
    @GET("api/my-crops/{id}")
    fun getCropDetail(@Path("id") id: kotlin.Long): Single<ResultMyCropResponseDto>

    /**
     * 获取当前用户资料
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @return [Call]<[ResultUserProfileDto]>
     */
    @GET("api/users/me")
    fun getCurrentUserProfile(): Single<ResultUserProfileDto>

    /**
     * 获取我收藏的帖子列表
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultPageResultPostResponseDto]>
     */
    @GET("api/posts/favorites")
    fun getFavoritedPosts(@Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultPageResultPostResponseDto>

    /**
     * 获取当前用户的识别历史记录
     * 获取当前登录用户以往调用 Agent 的历史记录，按时间倒序排列
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @return [Call]<[ResultListAgentChatHistory]>
     */
    @GET("api/v1/agent/history")
    fun getHistory(): Single<ResultListAgentChatHistory>

    /**
     * 获取消息发布历史记录
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultPageResultAdminMessage]>
     */
    @GET("api/admin/messages/history")
    fun getMessageHistory(@Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultPageResultAdminMessage>

    /**
     * 获取我的作物列表
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @return [Call]<[ResultListMyCropResponseDto]>
     */
    @GET("api/my-crops")
    fun getMyCrops(): Single<ResultListMyCropResponseDto>

    /**
     * 获取消息列表 (分页)
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultMessageGroupResponseDto]>
     */
    @GET("api/messages")
    fun getMyMessages(@Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultMessageGroupResponseDto>

    /**
     * 获取我发布的帖子列表
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultPageResultPostResponseDto]>
     */
    @GET("api/posts/mine")
    fun getMyPosts(@Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultPageResultPostResponseDto>

    /**
     * 获取指定帖子详情
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultPostResponseDto]>
     */
    @GET("api/posts/{id}")
    fun getPostDetail(@Path("id") id: kotlin.Long): Single<ResultPostResponseDto>

    /**
     * 获取帖子列表
     * 支持按标签过滤、关键字搜索和分页
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param tag  (optional)
     * @param q  (optional)
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultPageResultPostResponseDto]>
     */
    @GET("api/posts")
    fun getPosts(@Query("tag") tag: kotlin.String? = null, @Query("q") q: kotlin.String? = null, @Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultPageResultPostResponseDto>

    /**
     * 实时获取搜索建议词
     * 用于搜索框输入时的实时下拉列表
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param q 
     * @return [Call]<[ResultListString]>
     */
    @GET("api/search/suggestions")
    fun getSuggestions(@Query("q") q: kotlin.String): Single<ResultListString>

    /**
     * 获取热门标签列表
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @return [Call]<[ResultListTagDto]>
     */
    @GET("api/tags")
    fun getTopTags(): Single<ResultListTagDto>

    /**
     * 获取未读消息数
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @return [Call]<[ResultLong]>
     */
    @GET("api/messages/unread-count")
    fun getUnreadCount(): Single<ResultLong>

    /**
     * 获取指定用户详情
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultAdminUserResponseDto]>
     */
    @GET("api/admin/users/{id}")
    fun getUserById(@Path("id") id: kotlin.Long): Single<ResultAdminUserResponseDto>

    /**
     * 获取用户列表 (分页)
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultPageResultAdminUserResponseDto]>
     */
    @GET("api/admin/users")
    fun getUsers(@Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultPageResultAdminUserResponseDto>

    /**
     * 健康检查
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @return [Call]<[ResultString]>
     */
    @GET("api/health")
    fun health(): Single<ResultString>

    /**
     * 给指定帖子点赞
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @POST("api/posts/{id}/like")
    fun likePost(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 用户登录
     * 使用用户名或手机号加上密码进行登录，成功后返回 JWT Token
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param loginRequest 
     * @return [Call]<[ResultAuthResponse]>
     */
    @POST("api/auth/login")
    fun login(@Body loginRequest: LoginRequest): Single<ResultAuthResponse>

    /**
     * 手机号验证码登录
     * 使用手机号和验证码进行登录，如果手机号未注册将自动注册
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param smsLoginRequest 
     * @return [Call]<[ResultAuthResponse]>
     */
    @POST("api/auth/login-by-sms")
    fun loginBySms(@Body smsLoginRequest: SmsLoginRequest): Single<ResultAuthResponse>

    /**
     * 一键已读所有消息
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @return [Call]<[ResultVoid]>
     */
    @PUT("api/messages/read-all")
    fun markAllAsRead(): Single<ResultVoid>

    /**
     * 标记单条消息为已读
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @PUT("api/messages/{id}/read")
    fun markAsRead(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 发布新消息 (全员或定向预警)
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param adminMessageCreateRequest 
     * @return [Call]<[ResultVoid]>
     */
    @POST("api/admin/messages")
    fun publishMessage(@Body adminMessageCreateRequest: AdminMessageCreateRequest): Single<ResultVoid>

    /**
     * 刷新 Token
     * 使用有效的 Refresh Token 获取新的 Access Token
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param refreshTokenRequest 
     * @return [Call]<[ResultAuthResponse]>
     */
    @POST("api/auth/refresh")
    fun refresh(@Body refreshTokenRequest: RefreshTokenRequest): Single<ResultAuthResponse>

    /**
     * 用户注册
     * 提供用户名、手机号和密码进行注册，注册成功后自动下发 JWT Token
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param registerRequest 
     * @return [Call]<[ResultAuthResponse]>
     */
    @POST("api/auth/register")
    fun register(@Body registerRequest: RegisterRequest): Single<ResultAuthResponse>

    /**
     * 取消/删除种植标签
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @param tagOperationRequest 
     * @return [Call]<[ResultVoid]>
     */
    @HTTP(method = "DELETE", path = "api/my-crops/{id}/tags", hasBody = true)
    fun removeTag(@Path("id") id: kotlin.Long, @Body tagOperationRequest: TagOperationRequest): Single<ResultVoid>

    /**
     * 重发历史消息
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @POST("api/admin/messages/{id}/resend")
    fun resendMessage(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 高级模糊搜索帖子
     * 返回匹配结果、推荐内容及关键词建议
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param q 
     * @param page  (optional, default to 0)
     * @param size  (optional, default to 10)
     * @return [Call]<[ResultSearchResultResponse]>
     */
    @GET("api/search/posts")
    fun searchPosts(@Query("q") q: kotlin.String, @Query("page") page: kotlin.Int? = 0, @Query("size") size: kotlin.Int? = 10): Single<ResultSearchResultResponse>

    /**
     * 发送短信验证码
     * 使用阿里云短信认证服务发送验证码（如果未配置AK，则使用模拟发送并在控制台打印）
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param sendSmsRequest 
     * @return [Call]<[ResultVoid]>
     */
    @POST("api/auth/send-code")
    fun sendSmsCode(@Body sendSmsRequest: SendSmsRequest): Single<ResultVoid>

    /**
     * 初始化分片上传(断点续传)
     * 根据文件Hash检查上传进度，返回未上传分片的预签名URL。注意：MinIO/S3要求除了最后一个分片外，其他每个分片必须 &gt;&#x3D; 5MB。
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param multipartUploadInitRequest 
     * @return [Call]<[ResultMultipartUploadInitResponse]>
     */
    @POST("api/files/multipart/init")
    fun startMultipartUpload(@Body multipartUploadInitRequest: MultipartUploadInitRequest): Single<ResultMultipartUploadInitResponse>

    /**
     * 取消收藏
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/posts/{id}/favorite")
    fun unfavoritePost(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 取消关注用户
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/users/{id}/follow")
    fun unfollowUser(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 取消点赞
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @return [Call]<[ResultVoid]>
     */
    @DELETE("api/posts/{id}/like")
    fun unlikePost(@Path("id") id: kotlin.Long): Single<ResultVoid>

    /**
     * 管理员修改评论内容
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @param commentRequest 
     * @return [Call]<[ResultCommentResponseDto]>
     */
    @PUT("api/admin/community/comments/{id}")
    fun updateComment(@Path("id") id: kotlin.Long, @Body commentRequest: CommentRequest): Single<ResultCommentResponseDto>

    /**
     * 修改作物基本信息
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @param myCropUpdateRequest 
     * @return [Call]<[ResultMyCropResponseDto]>
     */
    @PUT("api/my-crops/{id}")
    fun updateCrop(@Path("id") id: kotlin.Long, @Body myCropUpdateRequest: MyCropUpdateRequest): Single<ResultMyCropResponseDto>

    /**
     * 修改登录密码
     * 修改成功后当前Token将立即失效，需重新登录
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param newPassword 
     * @return [Call]<[ResultVoid]>
     */
    @PUT("api/users/me/password")
    fun updatePassword(@Query("newPassword") newPassword: kotlin.String): Single<ResultVoid>

    /**
     * 管理员修改帖子内容
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @param postCreateRequest 
     * @return [Call]<[ResultPostResponseDto]>
     */
    @PUT("api/admin/community/posts/{id}")
    fun updatePost(@Path("id") id: kotlin.Long, @Body postCreateRequest: PostCreateRequest): Single<ResultPostResponseDto>

    /**
     * 更新当前用户资料
     * 支持部分更新，字段为空则不修改
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param profileUpdateRequest 
     * @return [Call]<[ResultUserProfileDto]>
     */
    @PUT("api/users/me")
    fun updateProfile(@Body profileUpdateRequest: ProfileUpdateRequest): Single<ResultUserProfileDto>

    /**
     * 管理员更新用户
     * 
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param id 
     * @param adminUserUpdateDto 
     * @return [Call]<[ResultAdminUserResponseDto]>
     */
    @PUT("api/admin/users/{id}")
    fun updateUser(@Path("id") id: kotlin.Long, @Body adminUserUpdateDto: AdminUserUpdateDto): Single<ResultAdminUserResponseDto>

    /**
     * 通用单文件上传
     * 上传单张图片/文件至对象存储，并返回可公开访问的 URL。
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param file 
     * @param prefix  (optional, default to "uploads/")
     * @return [Call]<[ResultString]>
     */
    @Multipart
    @POST("api/files/upload")
    fun uploadFile(@Part file: MultipartBody.Part, @Query("prefix") prefix: kotlin.String? = "uploads/"): Single<ResultString>

    /**
     * 通用多文件批量上传
     * 一次性上传多张图片/文件，返回按顺序排列的 URL 列表。
     * Responses:
     *  - 400: Bad Request
     *  - 500: Internal Server Error
     *  - 200: OK
     *
     * @param files 
     * @param prefix  (optional, default to "uploads/")
     * @return [Call]<[ResultListString]>
     */
    @Multipart
    @POST("api/files/upload/batch")
    fun uploadFiles(@Part files: List<MultipartBody.Part>, @Query("prefix") prefix: kotlin.String? = "uploads/"): Single<ResultListString>

}
